...
# Store active compression tasks
ACTIVE_TASKS = {}
    ...
    if data == 'help':
        ...
        buttons = [
            ...
            [InlineKeyboardButton("Cᴏᴍᴘʀᴇss 🗜️", callback_data=f"compress-{query.from_user.id}"), 
             InlineKeyboardButton("Cᴏᴍᴘʀᴇss 720ᴘ 🚀", callback_data=f"auto720p-{query.from_user.id}")]
        ]
        ...
        try:
            ...
            await query.message.edit('Added to queue for 720p compression...')
            asyncio.create_task(CompressVideo(bot=bot, query=query, ffmpegcode=ffmpeg, c_thumb=c_thumb))
        except Exception as e:
            print(e)
        ...
        try:
            ...
            await query.message.edit('Added to queue for 480p compression...')
            asyncio.create_task(CompressVideo(bot=bot, query=query, ffmpegcode=ffmpeg, c_thumb=c_thumb))
        except Exception as e:
            print(e)
        ...
        try:
            ...
            await query.message.edit('Added to queue for 1080p compression...')
            asyncio.create_task(CompressVideo(bot=bot, query=query, ffmpegcode=ffmpeg, c_thumb=c_thumb))
        except Exception as e:
            print(e)
        ...
        try:
            ...
            await query.message.edit('Added to queue for 4K compression...')
            asyncio.create_task(CompressVideo(bot=bot, query=query, ffmpegcode=ffmpeg, c_thumb=c_thumb))
        except Exception as e:
            print(e)
        ...
        try:
            ...
            if ffmpeg_code:
                await query.message.edit('Added to queue with custom compression...')
                asyncio.create_task(CompressVideo(bot=bot, query=query, ffmpegcode=ffmpeg_code, c_thumb=c_thumb))
            else:
                BUTT = [
